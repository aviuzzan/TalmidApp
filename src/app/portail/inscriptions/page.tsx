  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ background: facture.statut === 'solde' ? '#ECFDF5' : facture.statut === 'partiel' ? '#EFF6FF' : '#FFFBEB', color: facture.statut === 'solde' ? '#059669' : facture.statut === 'partiel' ? '#2563EB' : '#D97706', borderRadius: 20, padding: '4px 14px', fontSize: 12, fontWeight: 600 }}>
            ...
            <tbody>
              {lignes.map(l => (
                <tr key={l.id} style={{ borderTop: '1px solid #F1F5F9' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 500 }}>{l.enfants ? `${l.enfants.prenom || ''} ${l.enfants.nom || ''}`.trim() : '—'}</td>
