...
const PRICING = [
  {
    name: 'Découverte', price: 'Gratuit', period: '', desc: 'Pour tester la plateforme',
  ...
  {
    name: 'École', price: '49€', period: '/ mois', desc: 'Pour les petites et moyennes écoles',
    features: ['Familles illimitées', 'Tous les modules inclus', 'Support prioritaire', 'Admins illimités', 'Notifications email', 'Export PDF'],
    cta: 'Réserver ma place', highlight: true,
  },
  {
    name: 'Réseau', price: 'Sur devis', period: '', desc: "Pour les réseaux d'établissements",
    ...
    <div style={{ fontFamily: "'Outfit', 'DM Sans', system-ui, sans-serif", background: '#050A14', color: '#E8EDF8', overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #3B82F6, #8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: '#fff' }}>T</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
          {['Fonctionnalités', 'Tarifs', 'Contact'].map(item => (
            <a key={item} href={`#${item.toLowerCase()}`} style={{ color: 'rgba(255,255,255,0.6)', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>{item}</a>
          ))}
          <button onClick={() => router.push('/login')} style={{ background: 'rgba(59,130,246,0.15)', border: '1px solid rgba(59,130,246,0.4)', borderRadius: 8, padding: '8px 18px', color: '#60A5FA', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            Connexion →
          </button>
        </div>
      </nav>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '120px 5% 80px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 800 }}>
          <div className="fade-up-1" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 100, padding: '6px 16px', marginBottom: 32 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#22C55E', boxShadow: '0 0 8px #22C55E' }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: '#60A5FA', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>Disponible rentrée 2026–2027</span>
          </div>
          <h1 className="fade-up-2" style={{ fontSize: 'clamp(40px, 7vw, 80px)', fontWeight: 900, lineHeight: 1.05, letterSpacing: '-0.04em', marginBottom: 24 }}>
            La gestion scolaire<br /><span className="shimmer-text">réinventée.</span>
          </h1>
          <p className="fade-up-3" style={{ fontSize: 'clamp(16px, 2vw, 20px)', color: 'rgba(255,255,255,0.55)', lineHeight: 1.7, maxWidth: 580, margin: '0 auto 48px' }}>
          ...
          <div className="fade-up-4" style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="#contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg, #3B82F6, #6366F1)', borderRadius: 12, padding: '14px 28px', color: '#fff', fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: '0 8px 32px rgba(59,130,246,0.35)' }}>
              Réserver ma démonstration →
            </a>
            <a href="#fonctionnalites" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '14px 28px', color: 'rgba(255,255,255,0.8)', fontSize: 15, fontWeight: 600, textDecoration: 'none' }}>
        ...
        {/* Dashboard preview */}
        <div className="float" style={{ marginTop: 80, position: 'relative', zIndex: 1, width: '100%', maxWidth: 860, background: 'linear-gradient(135deg, rgba(30,50,80,0.8), rgba(15,25,45,0.9))', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: 24, boxShadow: '0 40px 80px rgba(0,0,0,0.6)' }}>
      ...
      <section style={{ padding: '60px 5%', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 32, textAlign: 'center' }}>
          {STATS.map(s => (
            <div key={s.label}>
              <div style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 900, letterSpacing: '-0.04em', color: '#60A5FA' }}>{s.value}</div>
      ...
      {/* FEATURES */}
      <section id="fonctionnalites" style={{ padding: '100px 5%' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 800, letterSpacing: '-0.03em' }}>Tout ce dont votre école a besoin</h2>
            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 16, marginTop: 16, maxWidth: 500, margin: '16px auto 0' }}>Une plateforme unifiée qui remplace vos fichiers Excel, vos mails et vos outils éparpillés.</p>
          </div>
      ...
      <section id="tarifs" style={{ padding: '100px 5%', background: 'rgba(255,255,255,0.01)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 800, letterSpacing: '-0.03em' }}>Simple et transparent</h2>
            <p style={{ color: 'rgba(255,255,255,0.45)', marginTop: 12 }}>Disponible pour la rentrée 2026–2027. Réservez votre place dès maintenant.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, alignItems: 'start' }}>
            {PRICING.map(plan => (
              <div key={plan.name} style={{ background: plan.highlight ? 'rgba(59,130,246,0.1)' : 'rgba(255,255,255,0.03)', border: plan.highlight ? '1px solid rgba(59,130,246,0.4)' : '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '32px 28px', transform: plan.highlight ? 'scale(1.03)' : 'scale(1)', position: 'relative' as const }}>
                {plan.highlight && <div style={{ position: 'absolute' as const, top: 16, right: 16, background: 'linear-gradient(135deg, #3B82F6, #6366F1)', borderRadius: 100, padding: '3px 12px', fontSize: 10, fontWeight: 700, color: '#fff' }}>POPULAIRE</div>}
                ...
                <a href="#contact" style={{ display: 'block', textAlign: 'center' as const, padding: '12px', borderRadius: 10, textDecoration: 'none', fontSize: 14, fontWeight: 600, background: plan.highlight ? 'linear-gradient(135deg, #3B82F6, #6366F1)' : 'rgba(255,255,255,0.07)', color: '#fff' }}>{plan.cta}</a>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="contact" style={{ padding: '100px 5%' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 15, marginBottom: 40 }}>Laissez-nous vos coordonnées, nous vous recontactons dans les 24h.</p>
          {submitted ? (
            <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 16, padding: '32px 24px' }}>
            ...
            <form onSubmit={handleContact} style={{ display: 'flex', flexDirection: 'column' as const, gap: 14, textAlign: 'left' as const }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginBottom: 6 }}>NOM DE L'ÉCOLE *</label>
                <input value={school} onChange={e => setSchool(e.target.value)} required placeholder="Ex: École Talmud Torah de Lyon" style={{ width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: '#fff', fontSize: 14, outline: 'none' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginBottom: 6 }}>EMAIL DE CONTACT *</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="directeur@votreecole.fr" style={{ width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: '#fff', fontSize: 14, outline: 'none' }} />
              </div>
              <button type="submit" style={{ padding: '14px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #3B82F6, #6366F1)', color: '#fff', fontSize: 15, fontWeight: 700 }}>
                Envoyer ma demande →
              </button>
            </form>
          )}
      ...
      {/* FOOTER */}
      <footer style={{ padding: '40px 5%', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
